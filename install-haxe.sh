...
if [ $RESP = "y" ]; then
	...
	if [ `uname -m` = "x86_64" ]; then
		...
		echo ""
		echo "---------------------------------------"
		echo "    Downloading Haxe $HAXE_VERSION Nightly Build (64-bit)"
		echo "---------------------------------------"	
		
		# Download nightly build
		#wget -c http://haxe.org/website-content/downloads/$HAXE_VERSION_COMMA/downloads/haxe-$HAXE_VERSION-linux64.tar.gz
		wget -c http://hxbuilds.s3-website-us-east-1.amazonaws.com/builds/haxe/linux64/haxe_2014-11-19_development_2dc77b9.tar.gz &&

		mv ./haxe_2014-11-19_development_2dc77b9.tar.gz ./haxe-$HAXE_VERSION-linux64.tar.gz 
		
		
		echo ""
		echo "---------------------------------------"
		echo "    Installing Haxe Nightly Build $HAXE_VERSION"
		echo "---------------------------------------"
		...
		sudo rm -rf /usr/bin/haxedoc-nightly
